...
> Última actualización: 2026-09-04 — **UX ciclo: toggle Ciclo|Mes + nombre del mes en el calendario** ✅ — `171/171` tests (22 files), `tsc --noEmit` 0. `PeriodPager` añade segmented `Ciclo|Mes` (permite volver a la vista mensual con el ciclo activo), `DateRangePicker` header usa `monthLabel(month)` → "Septiembre 2026". Version `0.4.1-finan`. Ver §4 (-19).
> Siguiente: **Cierre verificado OK** — quedó una app que apunta a Railway (`https://misgastos-production-b8c6.up.railway.app`); APK release instalado. **Nuevo (2026-09-04)**: UX del ciclo — toggle `Ciclo|Mes` en `PeriodPager` (volver a vista mensual con ciclo activo) + nombre del mes en el `DateRangePicker`; `171/171`, `tsc` 0, APK release instalado y verificado on-device (`uiautomator`: toggle + label `20 ago — 19 sep 2026` + header `Septiembre 2026`). Pendiente opcional: normalizar a mediodía los `date` de los gastos ya creados por ingesta (ver §4 -16) y evaluar `.limit(100)` si un mes supera 100 movimientos.
...
-19. **UX ciclo: toggle Ciclo|Mes + nombre del mes en el calendario** — 2026-09-04 — `mobile/App.tsx`, `mobile/src/components/ui/{PeriodPager,DateRangePicker}.tsx`, `mobile/src/screens/Presupuesto.tsx`:
    - **Problema**: con el ciclo activo (`billing_cycle_enabled=true`), `defaultPeriod()` era un rango, así que en el ciclo por defecto `isDefault===true` → `showClear=false` → el botón `×` (volver al default) nunca se mostraba; y aunque apareciera (rango custom), `resetToDefault()` volvía al **ciclo**, nunca a la vista mensual. Solo `Config` (toggle ciclo) permitía volver a mes. Además el header del `DateRangePicker` mostraba el mes como número (`"9 2026"`) en vez del nombre.
    - **Fix**: `PeriodPager.tsx` añade segmented control **`Ciclo | Mes`** (solo cuando `cycleEnabled`): `Mes` → `{type:"month", month:currentMonth()}`, `Ciclo` → `{type:"range", ...currentCycle(day)}`. `App.tsx` pasa `cycleEnabled`/`cycleRange` y oculta el `×` (`showClear={!isDefault && !cycleEnabled}`) porque el toggle cubre ambos modos. `DateRangePicker.tsx` usa `monthLabel(month)` de `tokens.ts` (en vez de `${m} ${y}`) → `"Septiembre 2026"`. `Presupuesto.tsx:116` hint corregido a "usa el selector «Mes» arriba para editar".
    - **Verificación**: `vitest 171/171 (22 files)` (sin cambios backend), `tsc --noEmit` 0. `assembleRelease` + `adb install -r` Success. `uiautomator dump`: toggle `CICLO`/`MES` presente, tap `CICLO` → label `20 ago — 19 sep 2026` y balance `$590.559`/`$811.656` (igual al screenshot del usuario), header del calendario `Septiembre 2026` + `Limpiar`/`Aplicar`.
    - **Nota**: el toggle es visible solo si el ciclo está activo; con ciclo inactivo el pager mantiene la etiqueta `mes` y el `×` (reset a mes) funciona como antes. Sin cambios de schema ni de backend.
...
-18. **Filtro de ciclo de facturación (20→20) + rango de fechas custom** — 2026-09-03 — `backend/src/modules/settings/routes.ts`, `backend/src/lib/dateRange.ts`, `backend/src/modules/balance/routes.ts`, `backend/src/modules/transactions/routes.ts`, `backend/src/modules/budgets/routes.ts`, `backend/src/lib/mockStore.ts`, `backend/src/index.ts`, `mobile/App.tsx`, `mobile/src/lib/useShellData.ts`, `mobile/src/lib/billingCycle.ts`, `mobile/src/components/ui/{PeriodPager,DateRangePicker,CyclePrompt}.tsx`, `mobile/src/screens/{Presupuesto,Config}.tsx`, `supabase/migrations/006_user_settings.sql`:
    ...
    - **Verificación**: `vitest 171/171 (22 files)` — nuevos `lib/dateRange.test.ts` (6), `modules/settings/routes.test.ts` (5), `modules/ranges/routes.test.ts` (3, mock supabase funcional). `tsc --noEmit` 0 errores. `/health` local `0.4.1-finan`. Migración 006 aplicada a Supabase (`user_settings` con RLS ok, registrada en `_migrations`).
    - **Deploy Railway**: push `36ef84f` (+ `4b0291c` idempotencia migración) → autodeploy OK. Health prod `0.4.1-finan`. Verificado prod: `GET /v1/settings` → `null` (ruta nueva) y `GET /v1/balance?from=2026-08-20&to=2026-09-19` → `range:{from:"2026-08-20",to:"2026-09-19",days:31}` con balance correcto.
...
- **Verificación**: `171/171` (22 files), `tsc` 0. Migración 006 aplicada a Supabase + **push → Railway autodeploy OK** (health prod `0.4.1-finan`, `/v1/settings` + `from`/`to` verificados en prod).
- **UX (2026-09-04)**: `PeriodPager` añade segmented **`Ciclo|Mes`** para volver a la vista mensual con el ciclo activo + `DateRangePicker` header muestra el nombre del mes (`Septiembre 2026`). Verificado on-device (APK release, `uiautomator`). Ver §4 (-19).
