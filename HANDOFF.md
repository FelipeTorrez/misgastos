...
> Siguiente: **Cierre verificado OK** — quedó una app que apunta a Railway (`https://misgastos-production-b8c6.up.railway.app`); APK release instalado con el fix de front. **Nuevo (2026-09-03)**: feature ciclo de facturación implementada (web/back) + migración 006 aplicada a prod + **push → Railway autodeploy OK** (health prod `0.4.1-finan`, `/v1/settings` + `from`/`to` verificados en prod). Pendiente opcional: normalizar a mediodía los `date` de los gastos ya creados por ingesta (ver §4 -16) y evaluar `.limit(100)` si un mes supera 100 movimientos.
